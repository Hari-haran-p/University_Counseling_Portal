...
const ExamPage = () => {
  ...
  useEffect(() => {
     const checkReadiness = async () => {
        ...
        const start = new Date(closestSchedule.start_time);
        const end = new Date(closestSchedule.end_time);
                ...
                {shuffledQuestions[currentQuestionIndex]?.options?.map((option) => (
                    <Button
                        ...
                        className={`text-black bg-gray-200 hover:bg-gray-300 ${
                            userAnswers[shuffledQuestions[currentQuestionIndex].id] === option ? "bg-green-500 text-white" : ""
                        }`}
